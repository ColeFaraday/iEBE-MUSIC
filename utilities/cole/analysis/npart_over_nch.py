...
def calculate_yield_and_meanpTsq(pT_low, pT_high, data):
    npT = 500
    ...
    for event_id in event_ids:
        ...
        for exp, cuts in kinematicCutsDict.items():
            ...
            try:
                ...
                Nch, meanpTsq = calculate_yield_and_meanpTsq(cuts["pTmin"], cuts["pTmax"], vn_data)
            except KeyError:
                Nch = -1.0
                meanpTsq = -1
            Nch_list.append(Nch)
            meanpTsq_list.append(meanpTsq)

        row = f"{id:<7d}  {centMin:<7.2f}  {centMax:<7.2f}  {Ncoll:<6d}  {Npart:<6d}  " + "  ".join(f"{nch:.4e}" for nch in Nch_list) + "  ".join(f"{mpt:.4e}" for mpt in meanpTsq_list)+ "\n"
        fout.write(row)
