...
try:
    ...
    # Check if a specific centrality config is requested
    if len(argv) > 2:
        config_key = argv[2]
        if config_key not in centrality_configs:
            print(f"Unknown centrality configuration: {config_key}")
            print(f"Available configurations: {list(centrality_configs.keys())}")
            exit(1)
    else:
        config_key = "default_dNdy"
        print(f"Using default centrality configuration: {config_key}")
    
    config = centrality_configs[config_key]
    print(f"Centrality configuration: {config['name']}")
    print(f"Description: {config['description']}")
    print(f"Using file: {config['file']}")
    ...
    print(f"Usage: {argv[0]} results_folder [centrality_config]")
...
# Extract centrality variable for all events
centrality_values = []
valid_events = []
...
print("total number of valid events: {}".format(len(centrality_values_sorted)))
...
for icen in range(len(centrality_cut_list) - 1):
    ...
    print(f"Centrality {centrality_cut_list[icen]}%-{centrality_cut_list[icen+1]}%:")
    print(f"  Variable range: {cent_cut_low:.3f} to {cent_cut_high:.3f}")
    ...
    print(f"  Number of events: {nev}")
    
    # Move events to centrality folders
...
# Save centrality mapping
save_centrality_mapping(data_path, config_key, event_centrality_map)

hf.close()
print(f"Centrality classification complete using '{config['name']}' method.")